<?php
 ...
 echo '<div class="container-fluid belka text-center"><h2>Lista salonów</h2></div><br />';
 echo '<div class="container-fluid">';
 if(isset($_SESSION['status'])=='administrator') {

//wyswietlenie salonow
$zapytanie="SELECT id_salonu, nazwa_salonu, ulica, miejscowosc, kod_pocztowy, nip, salon.nr_telefonu, opis, godziny_otwarcia, imie, nazwisko, akceptacja from salon inner join wniosek_salon on salon.id_wniosku=wniosek_salon.id_wniosku inner join uzytkownik on wniosek_salon.id_wlasciciela=uzytkownik.id_uzytkownika";
$wyk_zap=mysqli_query($naw_pol, $zapytanie);
 
if(mysqli_num_rows($wyk_zap)<1){
    
    echo 'Nie ma żadnych salonów.';
} else {
